pipeline{
    ...
    stages{
        ...
        stage('Docker Deploy'){
            steps{
              ansiblePlaybook credentialsId: 'apache-server', disableHostKeyChecking: true, extras: "-e DOCKER_TAG=${DOCKER_TAG}", installation: 'ansible2', inventory: 'dev.inv', playbook: 'deploy-docker.yml'
            }
        }
    }
}
